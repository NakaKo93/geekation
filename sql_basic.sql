...
-- 「Arubaの人口は103000人です」
SELECT CONCAT(name, 'の人口は', population, '人です') AS 'POPURATION' FROM countries;
...
SELECT SUBSTRING(name, 1, 1) AS 'initials', name FROM countries;

-- 問20
-- 国名が長いものから順に国名と国名の長さを出力してください。
SELECT name, CHAR_LENGTH(name) AS 'length' FROM countries ORDER BY length DESC;

-- 問21
-- 全ての地方の平均寿命、平均人口を表示してください。(NULLも表示)
SELECT region, AVG(life_expectancy) AS life_expectancy, AVG(population) AS population FROM countries GROUP BY region;

-- 問22
-- 全ての地方の最長寿命、最大人口を表示してください。(NULLも表示)
SELECT region, MAX(life_expectancy) AS life_expectancy, MAX(population) AS population FROM countries GROUP BY region;
...
SELECT SUM(surface_area) AS 'アジアの最小表面積' FROM countries WHERE continent = 'Asia' GROUP BY continent;

-- 問25
-- 全ての国と言語を表示してください。一つの国に複数言語があると思いますので同じ国名を言語数だけ出力してください。
SELECT name, language FROM countries RIGHT OUTER JOIN countrylanguages ON countries.code = countrylanguages.country_code;

-- 問26
-- 全ての国と言語と市区町村を表示してください。
SELECT countries.name AS '国名', cities.name AS '市区町村名', countrylanguages.language AS '言語' FROM countrylanguages 
LEFT OUTER JOIN countries ON countrylanguages.country_code = countries.code 
LEFT OUTER JOIN cities ON cities.country_code = countrylanguages.country_code 
ORDER BY countries.name ASC, cities.name ASC;

-- 問27
-- 全ての有名人を出力してください。左側外部結合を使用して国名なし（country_codeがNULL）も表示してください。
SELECT celebrities.name AS '名前', countries.name AS '国名' 
FROM celebrities 
LEFT OUTER JOIN countries ON celebrities.country_code = countries.code;

-- 問28
-- 全ての有名人の名前,国名、第一言語を出力してください。
SELECT 
    celebrities.name AS '名前', 
    countries.name AS '国名',
    countrylanguages.language AS '第一言語'
FROM celebrities
LEFT OUTER JOIN countries ON celebrities.country_code = countries.code
LEFT OUTER JOIN countrylanguages ON celebrities.country_code = countrylanguages.country_code
WHERE percentage IN (
    SELECT MAX(percentage) FROM countrylanguages
...
FROM countries
LEFT OUTER JOIN celebrities ON countries.code = celebrities.country_code
GROUP BY countries.name
HAVING count(celebrities.age) != 0
ORDER BY `平均年齢` DESC;