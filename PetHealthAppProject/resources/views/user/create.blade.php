...
<body class="bg-main">
    ...
    <div class="pre-main-box flex-vert bg-main">
        <div class="mx-auto mb-auto">
            <h1 class="txt-user-M fw-bold display-4">新規会員登録</h1>
        </div>
        <form class="flex-vert mx-auto w-75 h-75" action="{{ route('user.create-process') }}" method="POST">
            @csrf
            <div class="mt-auto">
                <label for="name" class="mb-1 fs-6 txt-user-S">氏名</label>
                <input type="text" class="px-2 w-100 fs-5 input_user_field txt-gray" id="name" name="name" value="{{ old('name', '') }}">
                @if ($errors->has('name'))
            ...
            <div class="mt-3">
                ...
                <input type="email" class="px-2 fs-5 w-100 input_user_field txt-gray" id="email" name="email" value="{{ old('email', '') }}">
                @if ($errors->has('email'))
            ...
            <div class="mt-3">
                ...
                @if ($errors->has('password'))
                    @foreach($errors->get('password') as $message)
                        <p class="mb-2 txt-error"> {{ $message }} </p>
                    @endforeach
                @endif
            </div>
            <div class="mt-auto text-center">
                <button class="py-1 fs-4 rounded-3 btn-user-action" type="submit">新規登録</button>
            </div>
        </form>
        <div class="mx-auto mt-3">
                ...
                @if (session('errorMessages'))
                    <p class="mb-2 txt-error" >{{ session('errorMessages') }}</p>
                @endif
            <p class="mx-auto mb-1 fs-6 txt-accent-S text-center">登録済みの方</p>
            <a class="py-1 fs-4 rounded-3 btn-accent-action" href="{{ route('user.login') }}">ログイン</a>
        </div>
    </div>
